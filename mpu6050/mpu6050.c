...
static int mpu6050_read_data(int _register)
{
	int temp;
	...
	temp = (s16)((u16)i2c_smbus_read_word_swapped(drv_client, _register));
...
{
	g_mpu6050_data.accel_values[0] = mpu6050_read_data(REG_ACCEL_XOUT_H);
...
static ssize_t accel_y_show(struct class *class,
			    struct class_attribute *attr, char *buf)
{
	g_mpu6050_data.accel_values[1] = mpu6050_read_data(REG_ACCEL_YOUT_H);
...
static ssize_t accel_z_show(struct class *class,
			    struct class_attribute *attr, char *buf)
{
	g_mpu6050_data.accel_values[2] = mpu6050_read_data(REG_ACCEL_ZOUT_H);
...
static ssize_t gyro_x_show(struct class *class,
			   struct class_attribute *attr, char *buf)
{
	g_mpu6050_data.gyro_values[0] = mpu6050_read_data(REG_GYRO_XOUT_H);
...
static ssize_t gyro_y_show(struct class *class,
			   struct class_attribute *attr, char *buf)
{
	g_mpu6050_data.gyro_values[1] = mpu6050_read_data(REG_GYRO_YOUT_H);
...
static ssize_t gyro_z_show(struct class *class,
			   struct class_attribute *attr, char *buf)
{
	g_mpu6050_data.gyro_values[2] = mpu6050_read_data(REG_GYRO_ZOUT_H);
...
static ssize_t temperature_show(struct class *class,
			 struct class_attribute *attr, char *buf)
{
	int temp = mpu6050_read_data(REG_TEMP_OUT_H);
